...
class mssql (
  ...
  $ps_check_sql_service = 'if($(get-service *MSSQL*)) {exit 0} else {exit 1}'
  User {
    ensure   => present,
  ...
  exec { 'install_mssql2008':
    command   => "${media}\\setup.exe /Action=Install /IACCEPTSQLSERVERLICENSETERMS /Q /CONFIGURATIONFILE=C:\\sql2008install.ini /SQLSVCPASSWORD=\"${sqlsvcpassword}\" /AGTSVCPASSWORD=\"${agtsvcpassword}\" /ASSVCPASSWORD=\"${assvcpassword}\" /RSSVCPASSWORD=\"${rssvcpassword}\" /SAPWD=\"${sapwd}\"",
    cwd       => $media,
    path      => $media,
    logoutput => true,
    unless    => "${powershell} ${ps_check_sql_service}",
    timeout   => 1200,
